 ...
 */
export function buildFlowJSON(state: FlowBuilderStateFrame, update: FlowPageLayoutElementV31, elementIndex: number, screenIndex: number, screen: FlowScreenV31): FlowJSONV31
{
  ...
  if (!allScreens || allScreens.length === 0) {
    ...
    const existingElements = screen.layout.children[0].children as FlowPageLayoutElementV31[];

    if (existingElements[elementIndex]) {
      existingElements[elementIndex] = update;
    } else {
